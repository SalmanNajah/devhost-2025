import { Flame, MicVocal } from "lucide-react";

export const markers = [
  {
    id: "day1",
    label: "Nov 6",
    events: [
      {
        id: "day1-event1",
        time: "08:30",
        displayTime: "8:30 AM - 10:00 AM",
        icon: <Flame className="h-auto sm:w-8" />,
        title: "Registration",
        description: "Attendee check-in and welcome desk.",
        venue: "TBA",
      },
      {
        id: "day1-event2",
        time: "10:00",
        displayTime: "10:00 AM - 10:45 AM",
        icon: <MicVocal className="h-auto sm:w-8" />,
        title: "Inaugural Keynote",
        description: "Kickoff keynote session to open DevHost 2025.",
        speaker: "TBA",
        role: "TBA",
        venue: "TBA",
      },
      {
        id: "day1-event3",
        time: "10:30",
        displayTime: "10:30 AM - 11:15 AM",
        icon: <MicVocal className="h-auto sm:w-8" />,
        title: "Beyond Code: Cultivating a Product Mindset",
        description:
          "Swapnil Agarwal will address bias in placements, ethical AI in hiring, and key strategies for interviews, skill-building, and career growth.",
        speaker: "Swapnil Agarwal",
        role: "Founder & CEO @ Cactro",
        venue: "TBA",
      },
      {
        id: "day1-event4",
        time: "11:30",
        displayTime: "11:30 AM - 12:15 PM",
        icon: <MicVocal className="h-auto sm:w-8" />,
        title: "DevTalk 2",
        description: "Technical talk session.",
        speaker: "TBA",
        role: "TBA",
        venue: "TBA",
      },
      {
        id: "day1-event5",
        time: "13:30",
        displayTime: "1:30 PM - 4:30 PM",
        icon: <MicVocal className="h-auto sm:w-8" />,
        title: "Exploring Polkadot: Foundations of Blockchain, FRAME Runtimes, and Dev Tools",
        description: "A beginner-friendly masterclass on blockchain fundamentals and Polkadot’s multi-chain architecture, featuring hands-on demos with Polkadot tools and insights into real-world developer opportunities..",
        speaker: "Raj Raorane",
        role: "Co-Founder of Edgetributors",
        venue: "TBA",
      },
      {
        id: "day1-event6",
        time: "16:30",
        displayTime: "4:30 PM - 5:00 PM",
        icon: <MicVocal className="h-auto sm:w-8" />,
        title: "Pre-Hackathon Talks",
        description: "Talks before the hackathon kickoff.",
        speaker: "TBA",
        role: "TBA",
        venue: "TBA",
      },
      {
        id: "day1-event7",
        time: "17:00",
        displayTime: "5:00 PM onwards",
        icon: <Flame className="h-auto sm:w-8" />,
        title: "DevHack - Hackathon Starts",
        description: "Official kickoff of the hackathon.",
        venue: "TBA",
      },
    ],
  },
  {
    id: "day2",
    label: "Nov 7",
    events: [
      {
        id: "day2-event1",
        time: "08:30",
        displayTime: "8:30 AM - 9:00 AM",
        icon: <Flame className="h-auto sm:w-8" />,
        title: "Registration",
        description: "Day 2 check-in.",
        venue: "TBA",
      },
      {
        id: "day2-event2",
        time: "09:00",
        displayTime: "9:00 AM - 9:45 AM",
        icon: <MicVocal className="h-auto sm:w-8" />,
        title: "The Cloud Isn't Safe — Until You Make It",
        description: "Technical talk session.",
        speaker: "N. Ananthakrishnan Potti",
        role: "Leading Security Operations @ OLA",
        venue: "TBA",
      },
      {
        id: "day2-event3",
        time: "10:00",
        displayTime: "10:00 AM - 10:45 AM",
        icon: <MicVocal className="h-auto sm:w-8" />,
        title: "Building in Open Source: Collaborate, Contribute, Create",
        description:
          "Aakansha Doshi will share her journey and guide students on the technical and practical aspects of contributing to open source",
        speaker: "Aakansha Doshi",
        role: "Core Maintainer and Contributor @ Project Excalidraw",
        venue: "TBA",
      },
      {
        id: "day2-event4",
        time: "11:00",
        displayTime: "11:00 AM - 12:30 PM",
        icon: <Flame className="h-auto sm:w-8" />,
        title: "R00TQuest - CTF",
        description: "Capture the Flag cybersecurity challenge.",
        venue: "TBA",
      },
      {
        id: "day2-event5",
        time: "11:00",
        displayTime: "11:00 AM - 12:30 PM",
        icon: <Flame className="h-auto sm:w-8" />,
        title: "How I Met My Investor",
        description: "Startup pitch competition.",
        venue: "TBA",
      },
      {
        id: "day2-event6",
        time: "09:00",
        displayTime: "9:00 AM - 5:00 PM",
        icon: <Flame className="h-auto sm:w-8" />,
        title: "Loot & Load",
        description: "Competitive Battle Royale Experience",
        venue: "TBA",
      },
      {
      id: "day2-event7",
      time: "14:00",
      displayTime: "1:00 PM - 4:00 PM",
      icon: <MicVocal className="h-auto sm:w-8" />,
      title: "Building Practical AI Agents: The Code Behind the Hype",
      description: "A hands-on workshop guiding you from basic AI agents to advanced multi-agent systems using core components like models, tools, orchestration, memory, and evaluation for real-world deployment.",
      speaker: "Charis Pinto & Samwin Pereira",
      role: "Presales Solution Architect and Solution Architect | Presales (Data Modernization & AI), Niveus Solutions",
      venue: "TBA",
      },
      {
        id: "day2-event8",
        time: "15:00",
        displayTime: "3:00 PM - 5:00 PM",
        icon: <Flame className="h-auto sm:w-8" />,
        title: "O(n) Slaught",
        description: "Competitive programming contest.",
        venue: "TBA",
      },
      {
        id: "day2-event9",
        time: "15:00",
        displayTime: "3:00 PM - 5:00 PM",
        icon: <Flame className="h-auto sm:w-8" />,
        title: "Oh My Grid!",
        description: "CSS grid based design contest.",
        venue: "TBA",
      },
    ],
  },
  {
    id: "day3",
    label: "Nov 8",
    events: [
      {
        id: "day3-event1",
        time: "05:00",
        displayTime: "5:00 AM - 6:00 AM",
        icon: <Flame className="h-auto sm:w-8" />,
        title: "DevHack - Hackathon Ends",
        description: "Submission deadline for hackathon projects.",
        venue: "TBA",
      },
      {
        id: "day3-event2",
        time: "07:00",
        displayTime: "7:00 AM - 10:00 AM",
        icon: <Flame className="h-auto sm:w-8" />,
        title: "Hackathon Final Judging",
        description: "Final evaluation of hackathon submissions.",
        venue: "TBA",
      },
      {
        id: "day3-event3",
        time: "09:00",
        displayTime: "9:00 AM - 10:30 AM",
        icon: <Flame className="h-auto sm:w-8" />,
        title: "Tech-Tac-Toe",
        description: "Interactive technical game event.",
        venue: "TBA",
      },
      {
        id: "day3-event4",
        time: "12:00",
        displayTime: "12:00 PM",
        icon: <MicVocal className="h-auto sm:w-8" />,
        title: "Valedictory",
        description: "Closing ceremony of DevHost 2025.",
        speaker: "TBA",
        role: "TBA",
        venue: "TBA",
      },
    ],
  },
];
